...
Statement *Parser::Stmt(bool consumeSemicolon)
{
    ...
    case Tag::INC:
    {
        ...
        stmt = new Increment(local); // Aqui, considere que você tem uma classe `Increment` que aceita uma expressão.
        return stmt;
    }
    // stmt -> local--;
    case Tag::DEC:
    {
        ...
        stmt = new Decrement(local); // Aqui, considere que você tem uma classe `Decrement` que aceita uma expressão.
        return stmt;
    }
    // stmt -> if (bool) stmt
    case Tag::IF:
