...
Increment::Increment(Expression *var) 
    : Statement(STMT), variable(var) 
{
    // Verifica se a variável é um identificador válido, se necessário
    if (variable->node_type != NodeType::IDENTIFIER) 
    {
        // Gerar erro se não for um identificador
        throw std::runtime_error("Increment operation is not applied to an identifier.");
    }
}

void Increment::Gen() 
{
    // Gerar o código intermediário ou de máquina para a operação de incremento
    cout << "\t" << variable->ToString() << " = " << variable->ToString() << " + 1" << endl;
}
...
Decrement::Decrement(Expression *var) 
    : Statement(STMT), variable(var) 
{
    // Verifica se a variável é um identificador válido, se necessário
    if (variable->node_type != NodeType::IDENTIFIER) 
    {
        // Gerar erro se não for um identificador
        throw std::runtime_error("Decrement operation is not applied to an identifier.");
    }
}

void Decrement::Gen() 
{
    // Gerar o código intermediário ou de máquina para a operação de decremento
    cout << "\t" << variable->ToString() << " = " << variable->ToString() << " - 1" << endl;
}